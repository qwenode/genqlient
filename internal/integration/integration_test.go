...
import (
    ...
    "github.com/qwenode/genqlient/internal/integration/server"
)
...
func TestCustomMarshal(t *testing.T) {
    ...
    for _, client := range clients {
        resp, _, err := queryWithCustomMarshal(
            ctx, client,
...
func TestCustomMarshalSlice(t *testing.T) {
    ...
    for _, client := range clients {
        resp, _, err := queryWithCustomMarshalSlice(
            ctx, client,
...
func TestInterfaceListField(t *testing.T) {
    ...
    for _, client := range clients {
        resp, _, err := queryWithInterfaceListField(
            ctx, client,
...
func TestInterfaceListPointerField(t *testing.T) {
    ...
    for _, client := range clients {
        resp, _, err := queryWithInterfaceListPointerField(
            ctx, client,
            []string{"1", "3", "12847394823"}
        )
        require.NoError(t, err)
...
//go:generate go run github.com/qwenode/genqlient genqlient.yaml
